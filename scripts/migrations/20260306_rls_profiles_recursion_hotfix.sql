-- Hotfix: avoid infinite recursion via profiles RLS when evaluating product/product_prices policies.
...
FOR INSERT
WITH CHECK (auth.role() = 'authenticated' AND public.is_staff_user());

CREATE POLICY "Products updatable by admins" ON public.products
FOR UPDATE
USING (auth.role() = 'authenticated' AND public.is_staff_user())
WITH CHECK (auth.role() = 'authenticated' AND public.is_staff_user());
